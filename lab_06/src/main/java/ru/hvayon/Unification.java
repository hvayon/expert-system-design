...
public class Unification {
    ...
    public boolean unification(Node firstAtom, Node secondAtom) {
        ...
        for (int i = 0; i < firstAtom.getArgs().size(); i++) {
            ...
            if (firstAtomParam.isConstant() && secondAtomParam.isVariable()) {
                ...
                // Если первый параметр является константой, а второй — переменной, то проверяется, имеет ли переменная уже привязанную константу.
                // Если да, то эта константа должна совпадать со значением первой константы. Если не совпадает - выходим
    ...
    public void addConstantLink(Variable variable, Constant constant) {
        ...
        // Цикл проходит по всем связанным переменным и устанавливает им ту же самую константу
